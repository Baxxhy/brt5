...
def _scrub_cloned_editable_installs(
    env_name: str, cwd: str, timeout: int
) -> dict[str, Any]:
    """Remove project worktree bindings from a cloned dependency environment."""
...
for metadata_dir in editable_metadata:
...
for pth in list(site.glob("*.pth")) + list(site.glob("*.egg-link")):
    try:
        ...
        continue
    external = strict_tdd_scrub and any(
...
def _clone_validated_dependency_environment(
    ...
    scrub = _scrub_cloned_editable_installs(target_env, cwd, timeout)
...
def _restore_runtime_dependency_contract(
    ...
    requirements = [
        ...
        and check.get("reason") in {"missing", "version_mismatch"}
        and check.get("requirement")
    ]
    package_names = [
        str(check.get("name"))
        for check in before.get("requirement_checks", [])
    ...
    install = _run_script(install_command, cwd, max(timeout, 600))
    repair = {
...
def ensure_isolated_runtime_environment(
    ...
    with open(lock_path, "w", encoding="utf-8") as lock:
        ...
        clone = _clone_validated_dependency_environment(
            template_env, target_env, workspace, timeout
        )
        if clone.get("returncode") != 0:
            return {
