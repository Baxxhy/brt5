...
def validate_runtime_contract(
    ...
    contract = {
        ...
        "evaluation_module": "brt5.evaluation.formal_eval",
...
def main() -> int:
    ...
    log_path = Path(args.log_path).resolve() if args.log_path else ROOT / "brt5/logs/formal_eval.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile("w", suffix=".json", encoding="utf-8", delete=False) as handle:
        json.dump(evaluation_rows, handle, ensure_ascii=False)
        filtered_path = handle.name
    command = [
        sys.executable, "-m", "brt5.evaluation.formal_eval",
